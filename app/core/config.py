...
class Config:
    ...
    TASK_FETCH_URL = os.getenv("TASK_FETCH_URL", f"{NODE_BASE}/task/alg_task_fetch")  # 前端的新增|修改任务的接口，从这里获取 enable的信息
    CONTROL_FETCH_URL = os.getenv("CONTROL_FETCH_URL", f"{NODE_BASE}/task/alg_task_control")  # 前端的停止|启动任务的接口，从这里获取 ControlCommand的信息

    # 上报警告给 node后端
    REPORT_URL = os.getenv("REPORT_URL", f"{NODE_BASE}/warning/alg_alarm_fetch")  # 前端的告警上报接口，从这里上报告警信息
